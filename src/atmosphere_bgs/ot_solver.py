...
class OTSolver:
    ...
    def initialise_weights_grid(self,top_scale=1.2,initial_weights_perturbation=1e-14):
    ...
    def initialise_weights_Voro(self, top_scale=1.2):
        '''
        ...
        # define weights
        E = np.array([(0.5 / pp.a**2) * self.y[:,0]**2, pp.cp * self.y[:,1]]).T
        ...
        # define final weight so that surface is above a multiple of plim
        thmin = np.min(self.y[:,1])
        ith = np.where(self.y[:,1]==thmin)
        iz = np.argmin(self.y[ith,0])
        ithz = ith[0][iz]
        zmin = self.y[ithz,0]
        phi_0 = phi[ithz] - self.c_KE(sp.smax,zmin) - self.c_enth(top_scale*plim[1],thmin)
    ...
    def get_bgs(self,
                ...
                min_area=1e-12, max_lost_areas=None,
                lr_up=2.0, lr_down=0.5, lr_max=1.0, lr_min=1e-20, lr_init=1e-5,
                initial_weights_perturbation=1e-14, top_scale=1.2):
        """
        ...
        if self.initial_weights is None:
            if self.interpolate_onto_grid:
                ...
                psi = self.initialise_weights_Voro(top_scale=top_scale)
