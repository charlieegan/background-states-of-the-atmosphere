...
rasterizer::segment::segment(const Eigen::Ref<const Eigen::Vector2d> &s,
...
std::string rasterizer::segment::repr(const int precision) const {
  std::stringstream ss;
  if (precision > 0)
...
inline double rasterizer::segment::slope() const {
  return (e(1) - s(1)) / (e(0) - s(0));
}

inline double rasterizer::segment::y_at(const double &x) const {
  return rasterizer::y_at(s(0), s(1), e(0), e(1), x);
}
inline double rasterizer::segment::x_at(const double &y) const {
  return rasterizer::x_at(s(0), s(1), e(0), e(1), y);
}

bool inline rasterizer::segment::intersect(segment &o, Eigen::Vector2d &p) {
...
void rasterizer::segment::bind(py::module_ &m) {
...
rasterizer::segment_cmp::segment_cmp(const rasterizer::segment_cmp &c) : cur_x(c.cur_x) {}
rasterizer::segment_cmp::segment_cmp(double *cur_x) : cur_x(cur_x) {}
...
rasterizer::event::event() : p(NULL) {}
rasterizer::event::event(rasterizer::segment &s, bool start) : p(static_cast<void*>(start ? &s.s : &s.e)) {}

// get the position of the event
inline const Eigen::Vector2d& rasterizer::event::pos() const {
  return *static_cast<Eigen::Vector2d*>(p);
}
// get whether the event represents a segment start
inline bool rasterizer::event::start() const {
  return !(reinterpret_cast<long long>(p) & 0x3Fll);
}
// get the segment
inline rasterizer::segment* rasterizer::event::seg() const {
  return reinterpret_cast<rasterizer::segment*>(reinterpret_cast<long long>(p) & ~0x3Fll);
}

bool operator<(const rasterizer::event &lhs, const rasterizer::event &rhs) {
  // lower x first
...
std::string rasterizer::event::repr(const int &precision) const {
  std::stringstream ss;
  if (precision > 0)
    ss << std::setprecision(precision);
  ss << "event("
...
void rasterizer::event::bind(py::module_ &m) {
...
rasterizer::rasterizer(rasterizer &&o) :
...
rasterizer::rasterizer(const std::vector<std::tuple<Eigen::Vector2d, Eigen::Vector2d, int>> &seg,
...
inline int rasterizer::x_to_pixel(const double &x) const {
  return std::floor((x - lb(0)) / step(0));
}
inline int rasterizer::y_to_pixel(const double &y) const {
  return std::floor((y - lb(1)) / step(1));
}
inline double rasterizer::pixel_to_x(const int &i) const {
  return lb(0) + i * step(0);
}
inline double rasterizer::pixel_to_y(const int &j) const {
  return lb(1) + j * step(1);
}

void rasterizer::add_area_between(const rasterizer::segment *s0, rasterizer::segment *s1, const double &x1) {
...
void rasterizer::bind(py::module_ &m) {
